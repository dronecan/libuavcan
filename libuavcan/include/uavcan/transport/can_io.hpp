...
class UAVCAN_EXPORT CanIOManager : Noncopyable
{
    ...
    TransferProtocol iface_protocol_[MaxCanIfaces];
...
public:
    ...
    TransferProtocol getIfaceProtocol(unsigned ifaceId);
    ...
    int send(const CanFrame& frame, TransferProtocol frame_protocol, MonotonicTime tx_deadline, MonotonicTime blocking_deadline,
             uint8_t iface_mask, CanTxQueue::Qos qos, CanIOFlags flags);
